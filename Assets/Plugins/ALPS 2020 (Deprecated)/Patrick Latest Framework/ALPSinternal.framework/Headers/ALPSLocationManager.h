 ...
 *  This initializer must be called for the location manager to connect to the Realm database for retreiving beacon configuration and floor plan data.
 *  @param remoteRealmIdentifier Remote Realm Identifer in the form /userid/realmid
 *  @param useRelativeLocation whether or not fuseRelativeLocation will be called
 *  @param solveIn3D whether to solve in 3D or 2D.  parameter only matters if useRelativeLocation is true.
 */
-(id)initWithRemoteRealmIdentifier:(NSString *)remoteRealmIdentifier useRelativeLocation:(BOOL)useRelativeLocation solveIn3D:(BOOL)solveIn3D;
 ...
 *  @param z Z coordinate of the current ARFrame provided by ARKit.
  * @param yaw Yaw Euler angle (y Euler angle) of the current ARFrame provided by ARKit.
 *  @param timeStamp Time stamp of the current ARFrame provided by ARKit.
 */
-(void)fuseRelativeLocationWithX:(double)x y:(double)y z:(double)z eulerX:(double)eulerX eulerY:(double)eulerY eulerZ:(double)eulerZ timeStamp:(double)timeStamp;
...
@property (nonatomic, readonly) BOOL useRelativeLocation;
