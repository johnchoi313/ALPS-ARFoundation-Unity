...
#define ALPS_REMOTE_REALM_IDENTIFIER @"/a813dda10478e48d283b237775d5ae55/Yodel"
...
- (id)init
{
    if (self = [super init])
    {
        ...
        // ------BEGIN ALPS------
        // Initialize ALPS
        alpsUnity = [[ALPSUnity alloc] initWithRemoteRealmIdentifier:ALPS_REMOTE_REALM_IDENTIFIER];
        // ------END ALPS------
    }
    return self;
}
...
- (void)session:(ARSession *)session didUpdateFrame:(ARFrame *)frame
{
    ...
    [alpsUnity fuseRelativeLocationWithX:frame.camera.transform.columns[3][0] y:frame.camera.transform.columns[3][1] locZ:frame.camera.transform.columns[3][2] eulerX:frame.camera.eulerAngles.x eulerY:frame.camera.eulerAngles.y eulerZ:frame.camera.eulerAngles.z timeStamp:frame.timestamp];
    // ------END ALPS------
...
- (void)session:(ARSession *)session cameraDidChangeTrackingState:(ARCamera *)camera
{
    ...
    //-------BEGIN ALPS-------
    //Send ARKit's camera tracking state to ALPS
    [alpsUnity setCameraTrackingState:camera];
    //-------END ALPS-------
}
